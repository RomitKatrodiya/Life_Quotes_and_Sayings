import 'package:life_quotes_and_sayings/helpers/quotes_api.dart';
...
class DBHelper {
  ...
  Future<List<QuotDB>> fetchAllRecords({required String tableName}) async {
    await insertRecord(tableName: tableName);
    String query = "SELECT * FROM $tableName";

    List<Map<String, dynamic>> allQuotes = await db!.rawQuery(query);

    List<QuotDB> quotes = allQuotes.map((e) => QuotDB.fromJSON(e)).toList();
